...
import { StreamConnectionEvents, Song, DMPError, DMPErrors } from "../index.js";
...
export class StreamConnection extends EventEmitter {
  ...
  createAudioStream(
    ...
    options: { inputType: StreamType; metadata?: any }
  ): AudioResource<Song> {
  ...
  setVolume(volume: number) {
    if (!this.resource || this._invalidVolume(volume)) return false;
  ...
  _invalidVolume(volume: number) {
    return isNaN(volume) || volume >= Infinity || volume < 0;
  }
}

export declare interface StreamConnection {
  on<K extends keyof StreamConnectionEvents>(
    event: K,
    listener: (...args: StreamConnectionEvents[K]) => void
  ): this;
}
