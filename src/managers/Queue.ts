...
export class Queue<T = unknown> {
  ...
  async play(
    ...
    if (options?.seek) song.seekTime = options.seek;
  ...
  async playlist(
    ...
    options: PlaylistOptions & { data?: T } = DefaultPlaylistOptions
  ): Promise<Playlist> {
    if (this.destroyed) throw new DMPError(DMPErrors.QUEUE_DESTROYED);
    if (!this.connection) throw new DMPError(DMPErrors.NO_VOICE_CONNECTION);
  ...
  move(oldIndex: number, newIndex?: number): boolean {
    ...
    if (oldIndex === newIndex) throw new DMPError(DMPErrors.INVALID_INDEX);
